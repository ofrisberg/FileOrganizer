import os,errno,logging,exifread,re
...
class Physical:
	def __init__(self,path):
		...
		self._dirname = os.path.basename(os.path.dirname(os.path.abspath(self._path)))
		self._mbytes = os.stat(self._path).st_size/1000000 #os.path.getsize(self._path)
	...
	def loadInfo(self):
		...
		if 'EXIF DateTimeOriginal' in exif:
			self._datetime = datetime.strptime(str(exif['EXIF DateTimeOriginal']), "%Y:%m:%d %H:%M:%S")
	...
	def _getDatetimeFromName(self):
		str = self._filename.rpartition('.')[0]
		match = re.search('(^|_|-)(19|20)(\d\d)(0[1-9]|1[012])(0[1-9]|[12][0-9]|3[01])(-|_|)([01][0-9]|[2][0-3])([0-5][0-9])([012345][0-9])($|_|-)',str)
		if match:
