...
class VirtualList:
	...
	def __init__(self,csv_path):
		if os.path.isfile(csv_path) is False:
			raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), csv_path)
	...
	def _loadData(self):
		...
		for datetime,path,camera,lat,lng,width,height,mbytes,orientation,old_dir,old_file in filereader:
			self._virtualfiles.append(Virtual(datetime,path,camera,lat,lng,width,height,mbytes,orientation,old_dir,old_file))
	...
	def write(self):
		...
		with open(self._csv_path,'w',newline='') as f:
			...
			w.writerow(["datetime","path","camera","lat","lng","width","height","mbytes","orientation","old_dir","old_file"])
			for file in self:
				w.writerow([file._datetime,file._path,file._camera,file._lat,file._lng,file._width,file._height,file._mbytes,file._orientation,file._old_dir,file._old_file])
