	...
	def _loadTuples(self):
		...
		while i_p < N_p and i_v < N_v:
			...
			if file_v.getName() == file_p.getName():
				...
				logging.error("Den virtuella filen till den fysiska '"+file_p.getName()+"' hittades inte")
				i_p += 1
				self._nr_missing_virtual += 1
			else:
				logging.error("Den fysiska filen till den virtuella '"+file_v.getName()+"' hittades inte i '"+self._list_v.getCSVPath()+"'")
				i_v += 1
