...
class FileOrganizer:
    ...
    def _getArgs(self):
        ...
        argparser.add_argument(
            'action', default=None, help="print_config/create_csv/check_corruption/src_stats/rename2creation/merge")
        argparser.add_argument(
            '-s', '--src-dir', default=None, help="Source directory")
        argparser.add_argument(
            '-d', '--dst-dir', default=None, help="Destination directory")
        argparser.add_argument(
            '-c', '--csv-file', default=None, help="CSV file")
        argparser.add_argument('-p', '--preview', action='store_true',
                               help="Preview changes without performing them")
        argparser.add_argument('-l', '--logging-level', default="warning",
                               help="debug/info/warning/error/critical, default='warning'")
    ...
    def merge(self):
        if self._args.src_dir is None:
            ...
            m = Merge(self._args.csv_file,
                      self._args.src_dir, self._args.dst_dir)
